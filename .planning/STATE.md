...
- [Phase 21]: 21-20: the W-A closure is attributed by carrier, not by class — 21-19 closed the passive-scan PROMPT carrier for HYPHENATED name shapes; Phase 27 closed the class (27-01 the second `sanitizeHeaders` carrier, 27-04/27-11/27-14/27-17 the CR/LF-escaped tool-result carrier, 27-10 `COOKIE_NAME_PART`'s exclusion of `_`). Residual (c) leaked on 21-19's own carrier because `sanitizeHeadersForPrompt` is an admitter and the regex was narrower than it — the difference set was fail-OPEN, the same asymmetry W-A itself was.
- [Phase 21]: 21-20: correcting the record does NOT tick PRIV-05 — `AR-27-08` is open and owned by Phase 28 (which accepted a further residual, D-28-09), so PRIV-05's "by any path" lead sentence is still stronger than what ships and stays `- [ ]`. Phase 21 SC1–SC6 remain 6/6; what was corrected is the claim layered on top, not the redaction work.
...
- 2026-08-28: Phase 21 plan record repaired — `21-19-PLAN.md` (third gap-closure round, W-A cookie header name variants on the prompt carrier, executed 2026-08-13) had never been recorded in `ROADMAP.md` or `STATE.md`, so no re-verification ever ran against it; phase-21 plan count corrected 18 → 19. The omission is why an incomplete fix was recorded as a complete closure of the W-A class and survived to the v0.10.0 milestone audit, becoming Phase 27's five rounds of rework (`21-VERIFICATION.md` G-2, repaired by plan 21-20).
