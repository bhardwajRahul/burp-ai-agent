# Requirements Archive: v0.9.0 — Hardening, Quality & New Capabilities

**Status:** SHIPPED 2026-06-26 — all 22 requirements satisfied
**Milestone audit:** `.planning/v0.9.0-MILESTONE-AUDIT.md` (status `passed`, 22/22)
**Archived:** 2026-08-05
...
|---------|--------------|
| Delivered as specified | PRIV-01, PRIV-02, PRIV-03, SEC-01, SEC-02, SEC-03, REL-01, REL-02, REL-03, REL-04, QUAL-01, QUAL-02, QUAL-03, QUAL-04, QUAL-05, CAP-02, CAP-03, CAP-04, DOC-01, DOC-02 |
| Delivered with agreed narrowing | PRIV-04 (save-time syntax/ReDoS validation instead of an interactive sample tester), CAP-01 (streaming + token counting + encrypted key + model selection; native tool-use and prompt-caching deferred) |
| Dropped | none |

### Post-ship note (2026-08-05 code review)

A deep review after the milestone shipped found that two v0.9.0 requirements are **satisfied in
design but incomplete in practice**. These are carried into v0.10.0 rather than reopened here:

- **PRIV-02** — body-level redaction misses cookie values re-emitted by the passive scanner without
  their `Cookie:` prefix (`JSESSIONID`, `PHPSESSID`, `connect.sid`, `auth_token`, `csrftoken` all
  pass through in STRICT and BALANCED). Redaction also fails open above 1 MB.
- **REL-02 / MCP hardening** — the MCP server's auth, Origin/Host/Referer and security-header
  interceptor is registered after `routing{}` in the Ktor `Call` phase, so it never executes for
  matched routes; the external-mode bearer-token gate does not protect `POST /message`.
