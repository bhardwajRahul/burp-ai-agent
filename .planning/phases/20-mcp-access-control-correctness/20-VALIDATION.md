...
| SC4 | SEC-04 | — | the SC1–SC3 assertions fail against pre-fix `KtorMcpServerManager` | process gate | stash-proof recipe (RESEARCH §SC4 Option A) | ❌ W0 | ⬜ pending |
| SC5a | SEC-05 | — | server advertises the real build version, not `0.6.0` | unit | `--tests '*BuildFlags*'` | ❌ W0 | ⬜ pending |
| SC5b | SEC-05 (D-11) | — | `isValidHost("[::1]:9876", 9876)` → true; `isValidOrigin`/`isValidReferer` accept `http://[::1]:9876` | unit (reflection, existing style) | `--tests '*KtorMcpServerManagerSecurityTest'` | ✅ extend | ⬜ pending |
...
| SC6 | SEC-04 | — | `/__mcp/shutdown` → 401 without token, 200 with token (must not regress) | integration | `--tests '*McpServerIntegrationTest'` | ✅ exists | ⬜ pending |
...
| D-06..D-09 | SEC-04 | — | blocked-request Output line + audit event fire once then aggregate; header values sanitized (D-07) and hashed by default (D-10) | unit (pure) | `--tests '*BlockedRequestReporterTest'` | ❌ W0 | ⬜ pending |
...
|----------|-------------|------------|-------------------|
| `docs/mcp-hardening.md` reflects D-01/D-02 (health deliberately exempt, agent header local-only) | D-12 | Doc accuracy is a judgement call, not an assertion | Read §"External Access" item 4 and §"Verification" item 2; confirm neither still claims every request is bearer-validated |
...
- [ ] SC4 stash-proof executed and its output recorded in the plan/commit body
