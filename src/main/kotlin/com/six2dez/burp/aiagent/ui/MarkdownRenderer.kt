...
object MarkdownRenderer {
    ...
    fun toHtml(text: String, isDark: Boolean): String {
        ...
        html = html.replace(CODE_BLOCK_REGEX) { m ->
        ...
        // Inline code (`...`)
        html = html.replace(INLINE_CODE_REGEX) { m ->
        ...
        html = html.replace(BLOCKQUOTE_REGEX) { m ->
        ...
        html = html.replace(UNORDERED_LIST_REGEX) { m ->
            "<div style='margin:1px 0 1px 16px;'>\u2022 ${m.groupValues[1]}</div>"
        }

        // Ordered lists (1. item at start of line)
        html = html.replace(ORDERED_LIST_REGEX) { m ->
            "<div style='margin:1px 0 1px 16px;'>${m.groupValues[1]}. ${m.groupValues[2]}</div>"
        }

        // Bold (**...**)
        html = html.replace(BOLD_REGEX) { m ->
            "<b>${m.groupValues[1]}</b>"
        }

        // Italic (*...*)
        html = html.replace(ITALIC_REGEX) { m ->
            "<i>${m.groupValues[1]}</i>"
        }

        // Links [text](url)
        html = html.replace(LINK_REGEX) { m ->
